...
|     1. | Substrate module: pallet-user                                    | This module is mainly used to record the user's history of using modules, favorite modules, and usage rate, etc.                                                                                                                              |
|     2. | Substrate module: pallet-supplier                                | This module is mainly used for registering suppliers, heartbeat methods, obtaining historical evaluations, obtaining pool rewards, etc.                                                                                                       |
...
|     4. | Substrate module: pallet-model                                   | This module is the core of the entire system, used for creating models (only for suppliers), creating model versions, initiating model execution, initiating model quotation orders, paying for model orders, generating model results, etc.  |
...
|    0c. | Testing Guide                                                     | Core functions will be fully covered by unit tests to ensure functionality and robustness. In the guide, we will describe how to run these tests.                                                                                                                                                                                        |
|    0d. | Docker                                                            | We will provide a Dockerfile(s) that can be used to test all the functionality delivered with this milestone.                                                                                                                                                                                                                            |
|     1. | Substrate module: pallet-member                                   | This module is used to create a membership system, including membership plans created by administrators. Users can call to purchase membership plans, suppliers can join the membership plan to provide their own created models, machine suppliers can join the plan to perform calculations for creating models, and get pool rewards. |
|     2. | Substrate module: pallet-machine-supplier                         | This module is mainly used to create machine services that only run models, without worrying about model evaluations or scores affecting the machine's evaluation. However, if the running speed is slow or unable to run, it will be subject to corresponding penalties.                                                                |
